...
import tikzStyle from "../../components/styles/tikz.inline.scss";
import {
  FilePath,
  pathToRoot,
  slugTag,
...
const tikzPreamble = String.raw`\usepackage{tikz}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsfonts}
\usepackage{amstext}
...
function normalizeTikzSource(value: string) {
...
async function renderTikzToHtml(source: string) {
  try {
    if (tex2svg === null) {
      throw new Error("node-tikzjax export could not be resolved");
    }

    const svg = await tex2svg(normalizeTikzSource(source), {
      addToPreamble: tikzPreamble,
    });
    return `<figure class="tikzjax-wrapper">${svg}</figure>`;
  } catch (error) {
    const message =
      error instanceof Error ? escapeHtml(error.message) : "TikZ render failed";
    const escapedSource = escapeHtml(source);
    return `<figure class="tikzjax-wrapper tikzjax-error"><figcaption>TikZ render failed: ${message}</figcaption><pre class="tikzjax-source"><code class="language-tikz">${escapedSource}</code></pre></figure>`;
  }
}
...
> = (userOpts) => {
  ...
  return {
    ...
    markdownPlugins(ctx) {
      ...
      const resolveEmbeddedFilePath = (
        file: { data: { relativePath?: FilePath } },
        fp: string,
      ) => {
        const normalized = fp.replace(/\\/g, "/").trim();
        if (!normalized || path.posix.isAbsolute(normalized)) {
          return normalized as FilePath;
        }

        const currentFilePath = file.data.relativePath;
        if (!currentFilePath) {
          return normalized as FilePath;
        }
        ...
        const relativeCandidate = path.posix.normalize(
          path.posix.join(currentDir, normalized),
        );
        const fallbackCandidate = path.posix.normalize(normalized);
        const candidates = [...new Set([relativeCandidate, fallbackCandidate])];

        for (const candidate of candidates) {
          const fullPath = path.join(ctx.argv.directory, candidate);
          if (fs.existsSync(fullPath)) {
            return candidate as FilePath;
          }
        }

        return fallbackCandidate as FilePath;
      };
          ...
          if (opts.wikilinks) {
            replacements.push([
              ...
              (value: string, ...capture: string[]) => {
                ...
                // embed cases
                if (value.startsWith("!")) {
                  const resolvedFp = resolveEmbeddedFilePath(file, fp);
                  const ext: string = path.extname(resolvedFp).toLowerCase();
                  const url = slugifyFilePath(resolvedFp);
                  if (
                    [
                      ".png",
                      ".jpg",
        ...
        plugins.push(() => {
          return async (tree: Root, _file) => {
            ...
            visit(tree, "code", (node, index, parent) => {
              if (parent && index !== undefined && node.lang === "tikz") {
                tikzNodes.push({
                  index,
            ...
            for (const { index, parent, node } of tikzNodes.reverse()) {
              const html: Html = {
                type: "html",
                value: await renderTikzToHtml(node.value),
              };
